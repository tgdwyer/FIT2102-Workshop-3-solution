...
const deck$: Observable<Card>
 = from(suits)
   .pipe(mergeMap(s=>from(ranks)
                     .pipe(map(r=>new Card(s,r)))))
...
function randomInsert<T>(a:readonly T[],e:T): readonly T[] {
  const i = impureRandomNumberGenerator(a.length);
...
const shoe$: (deckCount: number) => Observable<Card>
 = deckCount => 
  ...
  //   operator    input_type        output_type          transform
       ...
       reduce     <Card,             readonly Card[]>   ( shuffleInsert, [] ),
       mergeMap   <readonly Card[],  Observable<Card>>  ( from<readonly Card[]> )
      )
...
const count$: (deckCount: number, cardStream: Observable<Card>) => Observable<Count>
 = (deckCount, cards$) =>
  cards$.pipe(scan((a,card) => {
    const cardsAvailable = deckCount * deckSize
    ,     runningCount = ['2','3','4','5','6'].includes(card.rank) ? a.runningCount + 1
                       : ['10','J','Q','K','A'].includes(card.rank) ? a.runningCount - 1
                       : a.runningCount
    ,     cardsDealt = a.cardsDealt + 1
    ,     decksRemaining = (cardsAvailable - cardsDealt) / deckSize
    ,     trueCount = Math.floor(runningCount / decksRemaining)
    return { card, runningCount, cardsDealt, trueCount }
  }, {runningCount:0, cardsDealt:0} as Count))
